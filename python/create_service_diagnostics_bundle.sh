...
readonly DOCKER_IMAGE="mesosphere/dcos-sdk-service-diagnostics:${VERSION}-python"
...
if is_development_mode; then
  echo "dcos-sdk-service-diagnostics repository detected, running in development mode"
  echo
  echo "In development mode all Python modules will be picked up from your current"
  echo "dcos-sdk-service-diagnostics repository instead of the Docker image's"
  echo "/dcos-sdk-service-diagnostics-dist directory that contains a static git checkout"
  echo
  set -x
